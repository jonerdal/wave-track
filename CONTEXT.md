...
**Max Speed**
The peak speed reached during a Session, derived from the activity's FIT data and snapshotted when recording stops. Surfacing it approximates the speed of the fastest wave. Shown on the Summary screen.
_Avoid_: Top speed
...
**Summary screen** — Shows total time (hero), GPS distance, and Max Speed. Saves the session on button press then exits. Back button enters the Discard Confirmation screen.
